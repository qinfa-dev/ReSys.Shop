...
namespace ReSys.Core.Domain.Inventories.StockTransfers;
...
public sealed class StockTransfer : Aggregate
{
        ...
        string? reference = null)
    {
        ...
        var transfer = new StockTransfer
        {
            ...
            Reference = reference,
            State = StockTransferState.Pending, // Initialize state
    ...
    public ErrorOr<Success> Cancel()
    {
        if (State == StockTransferState.Finalized)
            return Errors.AlreadyInTerminalState(State);

        var oldState = State;
        State = StockTransferState.Finalized;
        UpdatedAt = DateTimeOffset.UtcNow;
        AddDomainEvent(new Events.StockTransferStateChanged(Id, oldState, State));
        return Result.Success;
    }
        ...
        IReadOnlyDictionary<Variant, int> variantsByQuantity)
    {
        if (State != StockTransferState.Pending)
            return Errors.InvalidStateTransition(State, StockTransferState.Finalized); // Can only transfer a pending transfer
    ...
    {
        if (State != StockTransferState.Pending)
            return Errors.InvalidStateTransition(State, StockTransferState.Finalized); // Can only receive for a pending transfer
    ...
    public ErrorOr<Deleted> Delete()
    {
        ...
        AddDomainEvent(new Events.StockTransferDeleted(TransferId: Id));
