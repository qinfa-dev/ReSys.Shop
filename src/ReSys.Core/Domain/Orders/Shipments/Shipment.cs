...
public sealed class Shipment : Aggregate
{
        ...
        if (onHandCount == 0)
        {
            ...
            if (backordered > 0)
            {
                return Error.Validation(
                    ...
                    description:
                    $"Shipment contains only backordered items ({backordered}). No items available to allocate.");
            }
    ...
    private static string GenerateShipmentNumber() =>
        $"S{DateTimeOffset.UtcNow:yyyyMMdd}{Random.Shared.Next(1000, 9999)}";
    ...
    public ErrorOr<Shipment> FinalizeShipment()
    {
        ...
        AddDomainEvent(new Events.ReadyToShip(
            ShipmentId: Id,
            OrderId: OrderId,
            StockLocationId: StockLocationId));
