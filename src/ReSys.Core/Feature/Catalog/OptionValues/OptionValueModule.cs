...
public static partial class OptionValueModule
{
    public sealed class Endpoints : ICarterModule
    {
        ...
        {
            var group = app.MapGroup(prefix: "api/admin/catalog/option-values")
                ...
                CancellationToken cancellationToken) =>
            {
                ...
                var result = await mediator.Send(request: command,
                    cancellationToken: cancellationToken);
                var apiResponse =
                    result.ToApiResponseCreated(message: "Option value created successfully");
                return TypedResults.Ok(value: apiResponse);
            })
                .UseEndpointMeta(meta: Annotations.Create)
                .RequireAccessPermission(permission: FeaturePermission.Admin.Catalog.OptionValue.Create);
                ...
                CancellationToken cancellationToken) =>
            {
                ...
                var result = await mediator.Send(request: command,
                    cancellationToken: cancellationToken);
                var apiResponse = result.ToApiResponseCreated(message: "Option value updated successfully");
                return TypedResults.Ok(value: apiResponse);
            }).UseEndpointMeta(meta: Annotations.Update).RequireAccessPermission(permission: FeaturePermission.Admin.Catalog.OptionValue.Update);
                ...
                CancellationToken cancellationToken) =>
            {
                ...
                var result = await mediator.Send(request: command,
                    cancellationToken: cancellationToken);
                var apiResponse = result.ToApiResponseDeleted(message: "Option value deleted successfully");
                return TypedResults.Ok(value: apiResponse);
            }).UseEndpointMeta(meta: Annotations.Delete)
                .RequireAccessPermission(permission: FeaturePermission.Admin.Catalog.OptionValue.Delete);
                ...
                CancellationToken cancellationToken) =>
            {
                ...
                var result = await mediator.Send(request: query,
                    cancellationToken: cancellationToken);
                var apiResponse = result.ToApiResponse(message: "Option values retrieved successfully");
                return TypedResults.Ok(value: apiResponse);
            }).UseEndpointMeta(meta: Annotations.GetSelectList)
                .RequireAccessPermission(permission: FeaturePermission.Admin.Catalog.OptionValue.List);
                ...
                CancellationToken cancellationToken) =>
            {
                ...
                var result = await mediator.Send(request: query,
                    cancellationToken: cancellationToken);
                var apiResponse = result.ToApiResponse(message: "Option value retrieved successfully");
                return TypedResults.Ok(value: apiResponse);
            }).UseEndpointMeta(meta: Annotations.GetById)
            .RequireAccessPermission(permission: FeaturePermission.Admin.Catalog.OptionValue.View);
