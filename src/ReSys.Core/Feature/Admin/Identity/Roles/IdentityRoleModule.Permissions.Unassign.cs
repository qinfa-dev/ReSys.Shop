            ...
            ) : ICommandHandler<Command, Success>
            {
                public async Task<ErrorOr<Success>> Handle(Command command, CancellationToken cancellationToken)
                {
                    try
                    {
                        ...
                        // Create claim object
                        var claimToRemove =
                            new System.Security.Claims.Claim(CustomClaim.Permission, command.Request.ClaimValue);

                        // Check if claim exists for the role
                        IList<System.Security.Claims.Claim> existingClaims = await roleManager.GetClaimsAsync(role);
                        System.Security.Claims.Claim? claim = existingClaims.FirstOrDefault(c =>
                        ...
                        if (claim == null)
                            return Error.NotFound(
                                ...
                                description:
                                $"Permission '{command.Request.ClaimValue}' is not assigned to role '{role.Name}'.");
                        ...
                        if (!result.Succeeded)
                        {
                            ...
                            logger.LogError(
                                message: "Failed to unassign permission {ClaimValue} from role {RoleId}: {Errors}",
                                args: [command.Request.ClaimValue, command.RoleId, errors]);

                            return Error.Failure(code: "Role.PermissionUnassignmentFailed",
                                description: $"Failed to unassign permission from role: {errors}");
                        }
                        ...
                        logger.LogInformation(
                            message: "Successfully unassigned permission {ClaimValue} from role {RoleId}",
                    ...
                    catch (Exception ex)
                    {
                        logger.LogError(exception: ex,
                            message: "Unexpected error unassigning permission {ClaimValue} from role {RoleId}",
                            args: [command.Request.ClaimValue, command.RoleId]);
                        return Role.Errors.UnexpectedError(operation: "unassigning permission from role");
                    }
                }
            }
        }
