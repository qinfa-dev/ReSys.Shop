...
public sealed class QueryFilterExtensionsTests(ITestOutputHelper output)
{
    ...
    public void ApplyFilters_WithNullQueryParams_ThrowsArgumentNullException()
    {
        ...
        // Act & Assert
        ((Action)(() => query.ApplyFilters((Dictionary<string, string>)null!))).Should().Throw<ArgumentNullException>();
    }
    ...
    public void ApplyFilters_WithContainsOperator_FiltersCorrectly()
    {
        ...
        // Assert
        result.Count().Should().Be(2); // John Doe and Bob Johnson
        result.All(u => u.Name.Contains("john", StringComparison.OrdinalIgnoreCase)).Should().BeTrue();
    }
    ...
    public void ApplyFilters_WithOrLogic_FiltersCorrectly()
    {
        ...
        // Assert
        result.Count().Should().Be(4); // John, Jane, Bob, Charlie (John contains john OR age >= 30)
    }
    ...
    public void ApplyFilters_WithAndLogic_FiltersCorrectly()
    {
        ...
        // Assert
        result.Count().Should().Be(1); // Only John (IT AND active)
    ...
    public void ApplyFilters_WithInOperator_FiltersCorrectly()
    {
        ...
        // Assert
        result.Count().Should().Be(4); // John, Jane, Bob, Charlie
        result.All(u => u.Department == "IT" || u.Department == "HR").Should().BeTrue();
    }
    ...
    public void ApplyFilters_WithNotInOperator_FiltersCorrectly()
    {
        ...
        // Assert
        result.Count().Should().Be(1); // Only Alice (Finance)
        result.First().Name.Should().Be("Alice Brown");
    }
    ...
    public void ApplyFilters_WithRangeOperator_FiltersCorrectly()
    {
        ...
        // Assert
        result.Count().Should().Be(4); // John (25), Jane (30), Alice (28), Charlie (32)
        result.All(u => u.Age >= 25 && u.Age <= 32).Should().BeTrue();
    }
    ...
    public void ApplyFilters_WithNotNullCheck_FiltersCorrectly()
    {
        ...
        // Assert
        result.Count().Should().Be(4); // All except Alice
        result.All(u => u.Address != null).Should().BeTrue();
    }
    ...
    public void ApplyFilters_WithComplexMixedLogic_FiltersCorrectly()
    {
        ...
        result.Select(u => u.Name).AsEnumerable().Should().Contain(["John Doe", "Jane Smith"]);
    }
    ...
    public void ApplyFilters_WithGlobalLogicOperator_FiltersCorrectly()
    {
        ...
        // Assert
        result.Count().Should().Be(2); // John (name match) OR Jane (HR department)
        result.Select(u => u.Name).OrderBy(n => n).Should().Contain(["Jane Smith", "John Doe"]);
    }
    ...
    public void ApplyFilters_WithInvalidOperator_IgnoresFilter()
    {
        ...
        result.Count().Should().Be(5); // All users have age > 20
    }
    ...
    public void ApplyFilters_WithInvalidPropertyName_IgnoresFilter()
    {
        ...
        result.Count().Should().Be(2); // Jane and Bob have age > 30
    }
    ...
    public void ApplyFilters_WithEmptyQueryString_ReturnsOriginalQuery()
    {
        ...
        // Act & Assert
        query.ApplyFilters("").Count().Should().Be(5);
        query.ApplyFilters("   ").Count().Should().Be(5);
        query.ApplyFilters((string)null!).Count().Should().Be(5);
    }
    ...
    public void ApplyFilters_WithNullQuery_ThrowsArgumentNullException()
    {
        ...
        // Act & Assert
        ((Action)(() => query.ApplyFilters("name[eq]=John"))).Should().Throw<ArgumentNullException>();
    }
    ...
    public void ApplyFilters_WithGlobalLogicQueryString_FiltersCorrectly()
    {
        ...
        // Assert
        result.Count().Should().Be(4); // Alice (Finance) OR users with age >= 30
    }
    ...
    public void ApplyFilters_WithComplexQueryString_FiltersCorrectly()
    {
        ...
        // Assert
        result.Count().Should().Be(1); // Only John Doe matches: name contains 'o' AND age >= 25 AND isActive = true
    }
    ...
    public void ApplyFilters_WithInvalidQueryString_HandlesGracefully()
    {
        ...
        // Assert
        result.Count().Should().Be(5); // Should return all users (no valid filters applied)
    }
    ...
    public void ApplyFilters_WithEncodedValues_DecodesCorrectly(string queryString, string expectedValue)
    {
        ...
        (user.Name == expectedValue || user.Email == expectedValue).Should().BeTrue();
    }
    ...
    public void ApplyFilters_StringVsDictionary_ProducesSameResults()
    {
        ...
        // Assert
        stringResult.Count().Should().Be(dictResult.Count());
        var stringUsers = stringResult.ToList();
        var dictUsers = dictResult.ToList();

        stringUsers.Count.Should().Be(dictUsers.Count);
        for (int i = 0; i < stringUsers.Count; i++)
        {
            stringUsers[i].Id.Should().Be(dictUsers[i].Id);
        }
    }
    ...
    public void ApplyFilters_WithSpecialCharactersInValue_HandlesCorrectly()
    {
        ...
        // Assert
        result.Count().Should().Be(1);
        result.First().Name.Should().Be("Test & User");
    }
